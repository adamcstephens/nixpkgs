...
in
{
  ...
  testScript = ''
    ...
    machine.succeed(
        "incus image import ${container-image-metadata}/*/*.tar.xz ${container-image-rootfs}/*/*.tar.xz --alias nixos"
    )
    ...
    with subtest("Container can be managed"):
        ...
        machine.succeed("incus delete -f container")

    with subtest("Container resources are correctly applied"):
        machine.succeed("incus launch nixos container")
        with machine.nested("Waiting for instance to start and be usable"):
            retry(instance_is_up)
