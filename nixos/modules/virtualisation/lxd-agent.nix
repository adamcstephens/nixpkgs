...
in {
  ...
  config = lib.mkIf config.virtualisation.lxd.agent.enable {
    # https://github.com/lxc/distrobuilder/blob/f77300bf7d7d5707b08eaf8a434d647d1ba81b5d/generators/lxd-agent.go#L153-L154
    ...
    systemd.services.lxd-agent = {
      unitConfig = {
        ...
        Before = lib.optionalString config.services.cloud-init.enable "cloud-init.target cloud-init.service cloud-init-local.service";
